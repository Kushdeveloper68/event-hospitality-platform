...
const getTeamMembers = async (eventId, filters = {}) => {
  try {
    ...
    if (filters.status && filters.status !== 'All') {
      // Map frontend 'Active'/'Inactive' to lowercase 'active'/'inactive'
      query.status = filters.status.toLowerCase();
    }

    if (filters.search) {
      // Case-insensitive regex search on both name and email
      query.$or = [
    ...
    const members = await TeamMemberModel.find(query).sort({ createdAt: -1 });
    return members;
  } catch (error) {
    throw error;
  }
