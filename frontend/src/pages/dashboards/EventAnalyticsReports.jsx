...
// ─── Progress bar row ─────────────────────────────────────────────────────────
export function ProgressRow({ label, value, pct, max, colorClass = "bg-blue-500", badge }) {
...
export function BarChart({ data = [], height = 160, colorClass = "fill-blue-500", labelKey = "label", valueKey = "count", showLabels = true }) {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${height + 28}`} width="100%" height={height + 28} preserveAspectRatio="none">
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x} y={height - barH} width={barW} height={barH} rx={3} className={colorClass} opacity={0.85} />
              {showLabels && data.length <= 24 && (
                <text x={x + barW / 2} y={height + 16} textAnchor="middle" fontSize="9" fill="#94a3b8">
                  {d[labelKey] !== undefined ? String(d[labelKey]).slice(0, 5) : i}
                </text>
              )}
              {val > 0 && (
                <text x={x + barW / 2} y={height - barH - 4} textAnchor="middle" fontSize="9" fill="#64748b">
                  {val}
                </text>
              )}
            </g>
...
// ─── SVG Line chart ───────────────────────────────────────────────────────────
export function LineChart({ data = [], height = 160, colorClass = "stroke-blue-500", labelKey = "label", valueKey = "count" }) {
  if (!data.length) return <EmptyChart />;
  const maxVal = Math.max(...data.map((d) => d[valueKey] ?? 0), 1);
  const W = 560;
  const H = height;
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H + 20}`} width="100%" height={H + 20} preserveAspectRatio="none">
        ...
        <path d={pathD} className={colorClass} strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        {pts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="3.5" fill="white" className={`${colorClass.replace("stroke-", "stroke-")}`} strokeWidth="2" stroke="#3b82f6" />
        ))}
        {data.map((d, i) => {
          if (i % Math.ceil(data.length / 8) !== 0) return null;
          return (
            <text key={i} x={pts[i].x} y={H + 16} textAnchor="middle" fontSize="9" fill="#94a3b8">
...
// ─── Donut chart ──────────────────────────────────────────────────────────────
export function DonutChart({ value, max, size = 120, colorClass = "stroke-blue-500", label, subLabel }) {
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative" style={{ width: size, height: size }}>
        <svg viewBox="0 0 100 100" width={size} height={size} className="-rotate-90">
          ...
          <circle cx="50" cy="50" r={r} fill="transparent" className={colorClass}
            strokeWidth="12" strokeDasharray={`${pct * circ} ${circ}`} strokeLinecap="round" />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-xl font-black text-slate-900 dark:text-white">{Math.round(pct * 100)}%</span>
        </div>
      </div>
      {label && <p className="text-xs font-bold text-slate-500 dark:text-slate-400 text-center">{label}</p>}
    ...
    <div className="space-y-2">
      <div className="flex items-end gap-0.5 h-20">
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-0.5" title={`${fmtHour(d.hour)}: ${d.count}`}>
              <div className={`w-full rounded-sm transition-all ${colorClass} opacity-80`} style={{ height: `${pct}%` }} />
            </div>
          );
        })}
      </div>
      ...
      {peak.count > 0 && (
        <p className="text-xs text-slate-500">
          Peak: <span className="font-bold text-slate-700 dark:text-slate-300">{fmtHour(peak.hour)}</span> ({fmtNum(peak.count)} activities)
        </p>
      )}
    </div>
  );
...
export default function EventAnalyticsReports() {
  ...
  // ── RENDER ────────────────────────────────────────────────────────────────
  return (
    <div className="max-w-[1400px] mx-auto space-y-5 pb-12 relative">

      {/* ── Toast ── */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-2xl shadow-xl font-semibold text-sm transition-all ${
          toast.type === "error"
      ...
      {activeTab === "overview" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <KPICard icon="star" label="VIP Guests" value={fmtNum(G?.summary?.vip)} sub={`${fmtPct(G?.summary?.vipRate)} of attendees`} accent="amber" />
            <KPICard icon="meeting_room" label="Rooms Assigned" value={fmtNum(G?.summary?.withRoom)} sub={`${fmtPct(G?.summary?.roomAssignmentRate)} assigned`} accent="purple" />
          </div>

          {/* KPI row 2 — ops */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard icon="pending_actions" label="Open Services" value={fmtNum((S?.summary?.open || 0) + (S?.summary?.inProgress || 0))} sub={`${fmtPct(S?.summary?.resolutionRate)} resolved`} accent={S?.summary?.open > 0 ? "rose" : "green"} />
            ...
            <KPICard icon="badge" label="Active Staff" value={fmtNum(TM?.summary?.active)} sub={`${fmtPct(TM?.summary?.activeRate)} on duty`} accent="teal" />
            <KPICard icon="schedule" label="Schedule Items" value={fmtNum(SC?.summary?.total)} sub={`${fmtNum(SC?.summary?.confirmed)} confirmed`} accent="blue" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <Section title="Check-in Activity by Hour" subtitle="When guests arrive throughout the day" icon="access_time">
              <HourBars data={G?.checkInByHour || []} colorClass="bg-blue-500" />
            </Section>
            <Section title="Service Request Types" subtitle="Volume by category" icon="room_service">
              {S?.byType?.length ? (
                <div className="space-y-3">
                  {S.byType.map((item) => (
                    <ProgressRow key={item.type} label={item.label} value={item.count} pct={item.percentage} colorClass="bg-blue-500" />
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <Section title="Room Occupancy" subtitle="Overall utilization rate" icon="meeting_room">
              <div className="flex flex-col items-center gap-4">
                <DonutChart value={R?.summary?.occupiedRooms || 0} max={R?.summary?.totalRooms || 1}
                  colorClass="stroke-blue-500" label="Rooms Occupied" subLabel={`${fmtNum(R?.summary?.occupiedRooms)} of ${fmtNum(R?.summary?.totalRooms)}`} />
            ...
            <Section title="Transport Status" subtitle="Fleet coordination" icon="local_shipping">
              <div className="space-y-3">
                {[
                  { label: "Scheduled", val: T?.summary?.scheduled, color: "bg-blue-400" },
                  { label: "In Transit", val: T?.summary?.inTransit, color: "bg-amber-400" },
                  { label: "Arrived", val: T?.summary?.arrived, color: "bg-emerald-500" },
                  { label: "Cancelled", val: T?.summary?.cancelled, color: "bg-slate-300 dark:bg-slate-600" },
                ].map((item) => (
                  <ProgressRow key={item.label} label={item.label} value={item.val || 0}
                    pct={T?.summary?.total > 0 ? Math.round(((item.val || 0) / T.summary.total) * 100) : 0}
            ...
            <Section title="Team & Activity" subtitle="Staff and operational logs" icon="groups">
              <div className="space-y-2">
                ...
                <StatRow label="Active" value={fmtNum(TM?.summary?.active)} valueClass="text-emerald-600" />
                <StatRow label="Inactive" value={fmtNum(TM?.summary?.inactive)} valueClass="text-slate-400" />
                <StatRow label="Total Activity Logs" value={fmtNum(A?.summary?.total)} />
                <StatRow label="Critical Today" value={fmtNum(A?.summary?.criticalToday)}
                  valueClass={A?.summary?.criticalToday > 0 ? "text-red-600" : ""} />
              </div>
            </Section>
          </div>
        </div>
      ...
      {activeTab === "guests" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard icon="group" label="Total Guests" value={fmtNum(G?.summary?.total)} accent="blue" />
            <KPICard icon="how_to_reg" label="Checked In" value={fmtNum(G?.summary?.checkedIn)} sub={fmtPct(G?.summary?.checkInRate)} accent="green" />
            <KPICard icon="logout" label="Checked Out" value={fmtNum(G?.summary?.checkedOut)} accent="slate" />
            <KPICard icon="star" label="VIP Guests" value={fmtNum(G?.summary?.vip)} sub={fmtPct(G?.summary?.vipRate)} accent="amber" />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* Check-in funnel */}
            <Section title="Guest Status Breakdown" subtitle="Arrival and check-in funnel" icon="how_to_reg"
              action={<ExportBtn onClick={() => handleExport("guests")} label="Export" loading={exporting.guests} />}>
              <div className="space-y-3">
                <ProgressRow label="Checked In" value={G?.summary?.checkedIn || 0} pct={G?.summary?.checkInRate || 0} colorClass="bg-emerald-500" />
                <ProgressRow label="Not Checked In" value={G?.summary?.notCheckedIn || 0}
                  pct={100 - (G?.summary?.checkInRate || 0)} colorClass="bg-slate-200 dark:bg-slate-700" />
                <ProgressRow label="VIP" value={G?.summary?.vip || 0} pct={G?.summary?.vipRate || 0} colorClass="bg-amber-400" />
                <ProgressRow label="Room Assigned" value={G?.summary?.withRoom || 0} pct={G?.summary?.roomAssignmentRate || 0} colorClass="bg-purple-400" />
                <ProgressRow label="Special Requests" value={G?.summary?.withSpecialRequests || 0}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Section title="Scheduled Arrivals by Hour" subtitle="Expected arrival distribution" icon="flight_land">
              <HourBars data={G?.arrivalByHour || []} colorClass="bg-indigo-400" />
            </Section>
              ...
              {G?.transportModeBreakdown?.length ? (
                <div className="space-y-3">
                  {G.transportModeBreakdown.map((item) => (
                    <ProgressRow key={item.mode} label={item.mode}
                      value={item.count}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Section title="Age Distribution" subtitle="Guest age group breakdown" icon="person">
              {G?.ageDistribution?.length ? (
                <BarChart data={G.ageDistribution} labelKey="range" valueKey="count" colorClass="fill-purple-400" height={140} />
                  ...
                  {G.topGroups.map((g, i) => (
                    <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{g.group}</span>
                      </div>
                      <span className="text-sm font-bold text-slate-900 dark:text-white tabular-nums">{fmtNum(g.count)}</span>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === "rooms" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard icon="meeting_room" label="Total Rooms" value={fmtNum(R?.summary?.totalRooms)} accent="blue" />
            <KPICard icon="hotel" label="Occupied" value={fmtNum(R?.summary?.occupiedRooms)} sub={fmtPct(R?.summary?.occupancyRate)} accent="green" />
            <KPICard icon="door_open" label="Available" value={fmtNum(R?.summary?.availableRooms)} accent="amber" />
            <KPICard icon="people" label="Guests Assigned" value={fmtNum(R?.summary?.totalGuestsAssigned)} sub={`Cap: ${fmtNum(R?.summary?.totalCapacity)}`} accent="purple" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Section title="Occupancy Rate" subtitle="Current room utilization" icon="meeting_room">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <DonutChart value={R?.summary?.occupiedRooms || 0} max={R?.summary?.totalRooms || 1}
                  size={140} colorClass="stroke-blue-500" label="Rooms Occupied" />
                <div className="flex-1 space-y-2 w-full">
                  <StatRow label="Occupancy Rate" value={fmtPct(R?.summary?.occupancyRate)} valueClass="text-blue-600" />
                  <StatRow label="Capacity Utilization" value={fmtPct(R?.summary?.capacityUtilization)} valueClass="text-purple-600" />
                  <StatRow label="Total Capacity" value={fmtNum(R?.summary?.totalCapacity)} />
              ...
              {R?.typeBreakdown?.length ? (
                <div className="space-y-3">
                  {R.typeBreakdown.map((item) => (
                    <ProgressRow key={item.type} label={item.type}
                      value={item.count}
      ...
      {activeTab === "services" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard icon="receipt_long" label="Total Requests" value={fmtNum(S?.summary?.total)} accent="blue" />
            <KPICard icon="pending_actions" label="Open" value={fmtNum(S?.summary?.open)} sub="Awaiting action" accent={S?.summary?.open > 0 ? "rose" : "green"} />
            <KPICard icon="run_circle" label="In Progress" value={fmtNum(S?.summary?.inProgress)} accent="amber" />
            <KPICard icon="check_circle" label="Resolved" value={fmtNum(S?.summary?.completed)} sub={fmtPct(S?.summary?.resolutionRate)} accent="green" />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* Type breakdown */}
            <Section title="By Service Type" subtitle="Request volume per category" icon="category"
              ...
              {S?.byUrgency?.length ? (
                <div className="space-y-3">
                  {S.byUrgency.map((item) => {
                    const colorMap = { emergency: "bg-red-600", high: "bg-red-400", medium: "bg-amber-400", low: "bg-emerald-400" };
                    return <ProgressRow key={item.urgency} label={item.urgency} value={item.count} pct={item.percentage}
            ...
            <Section title="Resolution Metrics" subtitle="Service completion health" icon="check_circle">
              <div className="flex flex-col items-center gap-4">
                <DonutChart value={S?.summary?.completed || 0} max={S?.summary?.total || 1}
                  size={120} colorClass="stroke-emerald-500" label="Resolution Rate" />
                <div className="w-full space-y-2">
                  <StatRow label="Open" value={fmtNum(S?.summary?.open)} valueClass="text-red-500" />
                  <StatRow label="In Progress" value={fmtNum(S?.summary?.inProgress)} valueClass="text-amber-500" />
                  <StatRow label="Completed" value={fmtNum(S?.summary?.completed)} valueClass="text-emerald-600" />
                  <StatRow label="Cancelled" value={fmtNum(S?.summary?.cancelled)} valueClass="text-slate-400" />
            ...
            <Section title="Recently Resolved Requests" subtitle="Last 5 completed service calls" icon="history" noPad>
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {S.recentResolved.map((req) => (
                  <div key={req._id} className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="size-8 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center">
                        <span className="material-symbols-outlined text-sm text-emerald-600">check_circle</span>
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-900 dark:text-white capitalize">{req.requestType}</p>
      ...
      {activeTab === "transport" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* Transports by hour */}
            <Section title="Scheduled Trips by Hour" subtitle="Fleet deployment throughout the day" icon="access_time"
                  ...
                  {T.topDrivers.map((d, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{d.driver}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-slate-900 dark:text-white">{d.trips}</span>
                  ...
                  {T.topRoutes.map((r, i) => (
                    <div key={i} className="flex items-start gap-3 py-2.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-slate-400 mb-0.5">Pickup → Dropoff</p>
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">
                          {r.pickup || "—"} → {r.dropoff || "—"}
                        </p>
                      </div>
                      <span className="text-sm font-bold text-slate-900 dark:text-white flex-shrink-0">{r.count}×</span>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === "team" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <KPICard icon="badge" label="Total Staff" value={fmtNum(TM?.summary?.total)} accent="blue" />
            <KPICard icon="check_circle" label="Active" value={fmtNum(TM?.summary?.active)} sub={fmtPct(TM?.summary?.activeRate)} accent="green" />
            <KPICard icon="person_off" label="Inactive" value={fmtNum(TM?.summary?.inactive)} accent="slate" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Section title="Staff Active Rate" subtitle="On-duty vs off-duty breakdown" icon="people">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <DonutChart value={TM?.summary?.active || 0} max={TM?.summary?.total || 1}
                  size={140} colorClass="stroke-emerald-500" label="Active Rate" />
                <div className="flex-1 space-y-3 w-full">
                  <ProgressRow label="Active" value={TM?.summary?.active || 0} pct={TM?.summary?.activeRate || 0} colorClass="bg-emerald-500" />
                  <ProgressRow label="Inactive" value={TM?.summary?.inactive || 0}
                    pct={100 - (TM?.summary?.activeRate || 0)} colorClass="bg-slate-200 dark:bg-slate-700" />
                </div>
              </div>
            </Section>
            ...
            <Section title="Staff by Role" subtitle="Role distribution across the team" icon="work">
              {TM?.byRole?.length ? (
                <div className="space-y-3">
                  {TM.byRole.map((item) => (
                    <ProgressRow key={item.role} label={item.role} value={item.count} pct={item.percentage} colorClass="bg-purple-400" />
                  ))}
                </div>
      ...
      {activeTab === "schedule" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard icon="event_note" label="Total Activities" value={fmtNum(SC?.summary?.total)} accent="blue" />
            <KPICard icon="check_circle" label="Confirmed" value={fmtNum(SC?.summary?.confirmed)} sub={fmtPct(SC?.summary?.completionRate)} accent="green" />
            <KPICard icon="pending" label="Pending" value={fmtNum(SC?.summary?.pending)} accent="amber" />
            <KPICard icon="cancel" label="Cancelled" value={fmtNum(SC?.summary?.cancelled)} accent="slate" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              {SC?.byWorkstream?.length ? (
                <div className="space-y-3">
                  {SC.byWorkstream.map((item) => {
                    const colorMap = {
                      "Main Sessions": "bg-blue-500", "Transport": "bg-amber-400",
                      "Catering": "bg-emerald-400", "Staffing": "bg-purple-400", "Media/AV": "bg-rose-400",
                    };
                    return <ProgressRow key={item.workstream} label={item.workstream} value={item.count}
                      pct={item.percentage} colorClass={colorMap[item.workstream] || "bg-slate-400"} />;
                  })}
                </div>
            ...
            <Section title="Schedule Status" subtitle="Activity completion pipeline" icon="schema">
              <div className="space-y-3">
                {[
                  { label: "Confirmed", val: SC?.summary?.confirmed, color: "bg-emerald-400" },
                  { label: "Active", val: SC?.summary?.active, color: "bg-blue-400" },
                  { label: "Pending", val: SC?.summary?.pending, color: "bg-amber-400" },
                  { label: "Cancelled", val: SC?.summary?.cancelled, color: "bg-slate-300 dark:bg-slate-600" },
                ].map((item) => (
                  <ProgressRow key={item.label} label={item.label} value={item.val || 0}
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {SC.upcomingActivities.map((act) => {
                  const workstreamColors = {
                    "Main Sessions": "border-l-blue-500", "Transport": "border-l-amber-500",
                    "Catering": "border-l-emerald-500", "Staffing": "border-l-purple-500", "Media/AV": "border-l-rose-500",
                  };
                  return (
                    <div key={act._id} className={`flex items-center gap-4 px-6 py-4 border-l-4 ${workstreamColors[act.workstream] || "border-l-slate-300"}`}>
                      <div className="flex-1 min-w-0">
      ...
      {activeTab === "activity" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <KPICard icon="history" label="Total Logs" value={fmtNum(A?.summary?.total)} accent="blue" />
            <KPICard icon="priority_high" label="Critical Today" value={fmtNum(A?.summary?.criticalToday)}
              accent={A?.summary?.criticalToday > 0 ? "rose" : "green"} />
            <KPICard icon="bar_chart" label="Unique Types" value={fmtNum(A?.byType?.length)} accent="purple" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Section title="Activity (Last 7 Days)" subtitle="Daily operational event volume" icon="trending_up">
              {A?.dailyTrend?.length ? (
                <BarChart data={A.dailyTrend} labelKey="date" valueKey="count" colorClass="fill-blue-400" height={150} />
              ...
              {A?.byType?.length ? (
                <div className="space-y-3">
                  {A.byType.map((item) => {
                    ...
                    return (
                      <ProgressRow key={item.type} label={item.type.replace(/-/g, " ")} value={item.count}
                        pct={Math.round((item.count / total) * 100)} colorClass="bg-blue-400" />
                    );
                  })}
                </div>
            ...
            {A?.byPriority?.length ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {A.byPriority.map((item) => {
                  const colorMap = { normal: "bg-blue-400", high: "bg-amber-400", critical: "bg-red-500" };
                  const iconMap = { normal: "info", high: "warning", critical: "emergency" };
                  const total = A.summary?.total || 1;
                  return (
                    <div key={item.priority} className="flex flex-col items-center gap-3">
                      <DonutChart value={item.count} max={total}
                        colorClass={`stroke-${item.priority === "critical" ? "red" : item.priority === "high" ? "amber" : "blue"}-500`}
                        label={item.priority} subLabel={`${fmtNum(item.count)} logs`} />
                    </div>
                  );
                })}
              </div>
